...
function buildActionSection() {
  return [
    ...
    "TENDER_ACCEPT         — params: { shipmentId, note? } — Accept a Tendered shipment on behalf of the carrier. Sets status='Tender Accepted', cascades to linked orders + CBOLs (MBOL), and runs the same OMS mirror + WS broadcast as the UI accept. USE THIS when the user says 'accept this tender' / 'confirm tender' / 'mark accepted'. Do NOT use UPDATE_SHIPMENT_STATUS for accepting — that path skips the OMS mirror and the linked-order cascade.",
    ...
    "- Safe actions (auto-executed): PLAN_ORDER, UPDATE_ORDER_STATUS, UPDATE_SHIPMENT_STATUS, ASSIGN_CARRIER, HOLD_ORDER, FLAG_EXCEPTION, ADD_SHIPMENT_EVENT, COPY_ORDER, GET_RATES, TENDER_ACCEPT, WITHDRAW_TENDER. Do NOT say 'click confirm' for these.",
