/**
 * RateUploadModal — Excel/CSV bulk-upload UI for the Rate Management page.
 *
 * Presentational layer only. Parsing, header-mapping, validation, conflict
 * detection, and persistence all live in services/rateUploadService.js
 * (CLAUDE_RULES §2 — services-first; §6/§9 — keep route handlers and
 * components free of large inline logic blocks).
 *
 * Flow
 *   1. user picks (or drops) a .xlsx/.csv file
 *   2. service parses + analyzes against existing rates
 *   3. preview shows total / valid / invalid / conflicts and the first
 *      rows so the user can sanity-check their sheet
 *   4. user picks how to handle conflicts (skip vs. overwrite)
 *   5. "Import N Rates" runs the chunked upsert with progress
 *   6. on done → toast + onComplete(refreshData)
 */
...
import {
  ...
  bulkUpsertRates,
  parseRateFile,
} from "../services/rateUploadService";
...
export default function RateUploadModal({ open, onClose, onComplete, existingRates = [] }) {
  const [file, setFile]             = useState(null);
  const [analysis, setAnalysis]     = useState(null);   // result of analyzeRateUpload
  const [conflictMode, setConflict] = useState("skip"); // "skip" | "overwrite"
  const [busy, setBusy]             = useState(false);
  const [progress, setProgress]     = useState({ completed: 0, total: 0 });
  const [stage, setStage]           = useState("idle"); // idle | parsing | preview | importing | done
  const [error, setError]           = useState("");
  const [result, setResult]         = useState(null);   // bulkUpsertRates summary
  const [dragOver, setDragOver]     = useState(false);
  const inputRef = useRef(null);

  const counts     = analysis?.counts || { total: 0, valid: 0, invalid: 0, conflicts: 0, newRows: 0 };
  const importable = useMemo(() => {
    if (!analysis) return 0;
    let n = analysis.analyzed.filter((a) => a.errors.length === 0).length;
    if (conflictMode === "skip") {
      n = analysis.analyzed.filter((a) => a.errors.length === 0 && !a.conflict).length;
    }
    return n;
  }, [analysis, conflictMode]);
  ...
  async function handlePickFile(picked) {
    ...
    setResult(null);
    try {
      const { headers, rows } = await parseRateFile(picked);
      if (rows.length === 0) {
        setError("The uploaded file has no data rows.");
        setStage("idle");
        return;
      }
      const a = analyzeRateUpload({ headers, rows, existingRates });
      setAnalysis(a);
      setStage("preview");
    } catch (err) {
  ...
  async function handleImport() {
    ...
    setError("");
    try {
      const summary = await bulkUpsertRates(analysis.analyzed, {
        conflictMode,
  ...
  function handleFinish() {
    ...
    if (onComplete) onComplete({ inserted, updated, skipped: result?.skipped || 0, failed: result?.failed || [] });
    reset();
    onClose && onClose();
  }
  ...
  const renderDropZone = () => (
    ...
    >
      ...
      <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text1)" }}>
        {stage === "parsing" ? "Reading file..." : "Click or drop your rate sheet here"}
      </div>
      ...
      <div style={{ fontSize: 11, color: "var(--text3)", marginTop: 12 }}>
        ...
        <button
          type="button"
          onClick={(e) => { e.stopPropagation(); downloadRateTemplate(); }}
    ...
    return (
      <div>
        <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
          ...
          <StatBlock label="NEW"       value={counts.newRows} color="#2563eb" />
          <StatBlock label="CONFLICTS" value={counts.conflicts} color="#b45309" />
        </div>
        ...
        {counts.conflicts > 0 && (
          <div style={{
            ...
            <div style={{ fontWeight: 600, fontSize: 12, marginBottom: 6 }}>
              {counts.conflicts} lane(s) already exist in the database
            </div>
            <label style={{ fontSize: 12, marginRight: 14, cursor: "pointer" }}>
              <input
                type="radio"
                name="conflictMode"
                value="skip"
          ...
          <table className="grid" style={{ border: "none", boxShadow: "none", width: "100%", fontSize: 11 }}>
            ...
            <tbody>
              {sampleRows.map((a) => {
                ...
                return (
                  <tr key={a.rowNumber} style={bad ? { background: "rgba(239,68,68,.05)" } : a.conflict ? { background: "rgba(245,158,11,.06)" } : undefined}>
                    ...
                    <td className="text-sm">
                      {bad ? (
                        ...
                        <span style={{ color: "#b45309", fontWeight: 600 }}>Lane exists</span>
                      ) : (
    ...
    return (
      <div style={{ padding: "20px 0", textAlign: "center" }}>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 12 }}>
          Importing {progress.completed} of {progress.total} rates...
        </div>
        <div style={{
  ...
  const renderDone = () => (
    <div>
      <div style={{ display: "flex", gap: 8, marginBottom: 12, flexWrap: "wrap" }}>
        <StatBlock label="INSERTED" value={result?.inserted || 0} color="#16a34a" />
        <StatBlock label="UPDATED"  value={result?.updated  || 0} color="#2563eb" />
        <StatBlock label="SKIPPED"  value={result?.skipped  || 0} color="#b45309" />
        ...
        <details open>
          ...
          <ul style={{ marginTop: 6, fontSize: 11, color: "var(--text2)", maxHeight: 160, overflow: "auto", paddingLeft: 18 }}>
            {result.failed.map((f, i) => (
              <li key={i}>
                <strong>Row {f.rowNumber}</strong> ({f.lane || "—"}): {f.error}
              </li>
            ))}
          </ul>
        </details>
    ...
    <div className="modal-overlay" onClick={handleClose}>
      <div className="modal-card" style={{ maxWidth: 760 }} onClick={(e) => e.stopPropagation()}>
        <div className="modal-header" style={{ alignItems: "center", gap: 12 }}>
          <h3 style={{ margin: 0, flex: 1 }}>UPLOAD RATES</h3>
          ...
          {stage === "preview" && (
            <>
              ...
              <button
                ...
                title={importable === 0 ? "Nothing to import — fix validation errors or change conflict mode" : ""}
              >
                Import {importable} Rate{importable === 1 ? "" : "s"}
              </button>
            </>
          )}
          {stage === "importing" && (
