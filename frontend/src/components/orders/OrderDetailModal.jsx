            ...
            <div style={{ padding: "20px 24px" }}>
              ...
              <div style={{ marginBottom: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12, marginBottom: 12 }}>
                  ...
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "var(--text3)", display: "block" }}>Mode</label><select value={editForm.shipMode || ""} onChange={(e) => onEditFormChange((f) => ({ ...f, shipMode: e.target.value }))} style={{ width: "100%", padding: "8px 10px", border: "1.5px solid var(--border)", borderRadius: 8, fontSize: 13, fontFamily: "inherit", background: "#fff", marginTop: 5 }}><option value="">— TMS selects —</option>{["TL", "LTL", "Intermodal", "Flatbed", "Reefer", "Parcel", "Expedite", "Air Freight"].map((m) => <option key={m} value={m}>{m}</option>)}</select></div>
                  {/* REQ-10: service-level dropdown acts as a planning constraint */}
                  <div><label style={{ fontSize: 11, fontWeight: 700, color: "var(--text3)", display: "block" }}>Service Level</label><select value={editForm.serviceLevel || ""} onChange={(e) => onEditFormChange((f) => ({ ...f, serviceLevel: e.target.value }))} style={{ width: "100%", padding: "8px 10px", border: "1.5px solid var(--border)", borderRadius: 8, fontSize: 13, fontFamily: "inherit", background: "#fff", marginTop: 5 }}><option value="">— TMS selects —</option>{["Standard", "Guaranteed", "Expedited", "Economy", "White Glove"].map((s) => <option key={s} value={s}>{s}</option>)}</select></div>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12, marginBottom: 12 }}>
