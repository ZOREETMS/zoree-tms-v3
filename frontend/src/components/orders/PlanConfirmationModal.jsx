        ...
        <div className="modal-body" style={{ overflowY: "auto", flex: 1 }}>
          ...
          <div style={{ padding: "12px 14px", background: "rgba(99,102,241,.04)", border: "1px solid rgba(99,102,241,.15)", borderRadius: 10, marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, opacity: dockEnabled ? 1 : 0.4, pointerEvents: dockEnabled ? "auto" : "none" }}>
              <select value={planModal.dockDoor || DOCK_DOORS[0]} onChange={(e) => onModalChange((p) => p ? { ...p, dockDoor: e.target.value } : null)} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid var(--border)", fontSize: 11, fontWeight: 600 }}>
                {DOCK_DOORS.map((d) => <option key={d} value={d}>{d}</option>)}
              </select>
              <select value={planModal.dockStartTime || "06:00"} onChange={(e) => onModalChange((p) => p ? { ...p, dockStartTime: e.target.value } : null)} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid var(--border)", fontSize: 11, fontWeight: 600 }}>
