...
export default function DockReservationSection({ dockEnabled, dockDoor, dockStartTime, loadDuration, onModalChange }) {
  return (
    <div style={{ padding: "12px 14px", background: "rgba(99,102,241,.04)", border: "1px solid rgba(99,102,241,.15)", borderRadius: 10, marginBottom: 12 }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8, opacity: dockEnabled ? 1 : 0.4, pointerEvents: dockEnabled ? "auto" : "none" }}>
        <select value={dockDoor || DOCK_DOORS[0]} onChange={(e) => onModalChange((p) => p ? { ...p, dockDoor: e.target.value } : null)} style={{ padding: "4px 8px", borderRadius: 6, border: "1px solid var(--border)", fontSize: 11, fontWeight: 600 }}>
