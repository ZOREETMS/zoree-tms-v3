...
export default function ExecuteRouteModal({ route, orders, onClose, onSuccess }) {
  ...
  // Auto-assign orders to CBOLs by matching origin→dest
  useEffect(() => {
    if (autoAssigned || cbolPairs.length === 0 || availableOrders.length === 0) return;
    const normalize = (s) => (s || "").trim().toLowerCase().replace(/\s+/g, " ");
    const newAssigning = {};
    for (const pair of cbolPairs) {
      const pLoc = normalize(pair.pickup.location || `${pair.pickup.city}, ${pair.pickup.state}`);
      const dLoc = normalize(pair.delivery.location || `${pair.delivery.city}, ${pair.delivery.state}`);
      const matched = availableOrders.filter((o) => {
        const oOrigin = normalize(o.origin);
        const oDest = normalize(o.dest);
        // Match if order origin contains pickup city and order dest contains delivery city
        const pCity = normalize(pair.pickup.city);
        const dCity = normalize(pair.delivery.city);
        return (oOrigin.includes(pCity) || pLoc.includes(normalize((o.origin || "").split(",")[0])))
          && (oDest.includes(dCity) || dLoc.includes(normalize((o.dest || "").split(",")[0])));
      });
      if (matched.length > 0) {
        newAssigning[pair.key] = matched.map((o) => o.id);
      }
    }
    if (Object.keys(newAssigning).length > 0) {
      setAssigning(newAssigning);
    }
    setAutoAssigned(true);
  }, [cbolPairs, availableOrders, autoAssigned]);
  ...
  async function execute() {
    ...
    try {
      ...
      // Compute dates from assigned orders
      const assignedOrders = allOrderIds.map((oid) => availableOrders.find((x) => x.id === oid)).filter(Boolean);
      ...
      const transitDays = parseInt(route.transit_days) || (lastDueDate && pickupDate
        ? Math.max(1, Math.round((new Date(lastDueDate) - new Date(pickupDate)) / 86400000))
        : null);
      const deliveryDate = lastDueDate || (transitDays
      ...
      const masterShipment = {
        ...
        // transit_days not in DB schema — computed from pickup/delivery dates in UI
      ...
      for (const pair of cbolPairs) {
        ...
        // Compute CBOL dates from its assigned orders
        const cbolOrders = orderIds.map((oid) => availableOrders.find((x) => x.id === oid)).filter(Boolean);
        ...
        const cbolTransit = cbolPickup && cbolDelivery
          ? Math.max(1, Math.round((new Date(cbolDelivery) - new Date(cbolPickup)) / 86400000))
          : transitDays;
